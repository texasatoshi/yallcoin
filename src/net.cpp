...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Yallcoin " + FormatFullVersion();
...
// cc, added digitalocean node as alpha node. 64.227.5.243
unsigned int pnSeed[] =
{
    0x0100007f,0xf305e340
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Yallcoin is probably already running."), addrBind.ToString().c_str());
        else
