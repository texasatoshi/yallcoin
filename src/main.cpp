...
#if defined(NDEBUG)
# error "Yallcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x599f3ad8b1ced6eeeea6bf85fcd2f64b09fcc28c06b4604862bfb58542ebfb1c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Yallcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Yallcoin Signed Message:\n";
...
{
    // Yallcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 52 * COIN; // increased from 50 -cc

    // Subsidy is cut in half every 520000 blocks, which will occur approximately every 3.5 years -cc
    nSubsidy >>= (nHeight / 520000); // Yallcoin: 520k blocks in ~3.5 years -cc

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Yallcoin: 3 days
static const int64 nTargetSpacing = 2 * 60; // Yallcoin: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Yallcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Yallcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xfb240722398b952d5598f304e42a1271fda1479e52522da4a80e88241797aaa8");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "ZeroHedge 6.24.2021 - John McAfee Found Dead In Prison Cell";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 52 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04202598dd5b7cefc812fa74d8e78a954430ab8f93f82889be4dcd8a4e592bf2935e927a977e62f028fed0b21c4ea1464506a088d30835abd051a8f6c9b76c2ca9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1624678499;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085583902;

        if (fTestNet)
        {
            block.nTime    = 1624678428;
            block.nNonce   = 385581817;
        }

//////////////////// from technocoin, generate genesis block -cc
...
///////////////////////// end genesis block generator
        ...
        assert(block.hashMerkleRoot == uint256("0x7def30f727becc37fa2d9d658e24b5b93deef2b1d282984fd60b5ee9c85d08c4"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc2, 0xb5, 0xde }; // Yallcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("YallcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("YallcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static YallcoinMiner(CWallet *pwallet)
{
    printf("YallcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("yallcoin-miner");
    ...
    try { loop {
        ...
        printf("Running YallcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("YallcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&YallcoinMiner, pwallet));
}
